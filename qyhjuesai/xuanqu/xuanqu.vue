<template>
	<view>
		<view class="colonn">
			<image @load="imgload($event,1)" 
			src="../../static/datitop.jpg" mode="widthFix" class="w-750"></image>
			...
			>
				<view class="colonn">
					<view  class="type1" 
					@click.stop="toda('东区')">东区</view>
					<view class="h-100"></view>
					<view  class="type1"
					@click.stop="toda('西区')">西区</view>
					<view class="h-100"></view>
					<view  class="type1"
					@click.stop="toda('南区')">南区</view>
					<view class="h-100"></view>
					<view  class="type1"
					@click.stop="toda('北区')">北区</view>
					<view class="h-100"></view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			toda(txt){
				uni.setStorageSync("qu",txt);
				uni.navigateTo({
					url:"/qyhjuesai/dafen/dafen"
				})
			},
			// 图片加载完成
			imgload(res,index){
